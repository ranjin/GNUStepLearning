...
//关于RunLoop的5个类(CFRunLoopModeRef并没有对外暴露,只是通过CFRunLoopRef的接口进行了封装)。
/**
 它们的关系是：一个runloop包含若干个Mode，每个mode又包含若干个Source/Timer/Observer，Source/Timer/Observer被成为mode item，一个item可以被同时加入多个mode，但一个item被重复加入同一个mode时是不会有效果的。如果一个mode中一个item都没有，则runloop会直接退出，不进入循环。 每次调用runloop的主函数时，只能指定其中一个mode，这个mode被称为currentMode。如果需要切换mode，只能先退出loop，再重新指定一个mode进入。这样做是为了分隔开不同组的Source/Timer/Observer，让其互不影响。
 */
...
/* CFRunLoopObserverRef可以观察到的几种状态
   Timer/Source/
 */
