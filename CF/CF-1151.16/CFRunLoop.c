...
#pragma mark - CFRunLoop的结构
/**
 有个概念叫“CommonModes”：一个Mode可以将自己标记为“Common”属性(通过将其ModeName添加到RunLoop的“commonModes”中)。每当runloop的内容发生变化时，runloop都会自动将_commonModeItems里的Source/Observer/Timer同步到具有"Common"标记的所有Mode里。
 */
struct __CFRunLoop {
    CFRuntimeBase _base;
...
/**
 ...
 Source0只包含了一个回调（函数指针），它并不能主动触发事件。使用时，需要先调用CFRunLoopSourceSignal（source）将source0标记为待处理，然后手动调用CFRunLoopWakeUp（runloop）来唤醒runloop，让其处理这个事件。
 
 Source1包含了一个mach_port和一个回调（函数指针），被用于通过内核和其它线程相互发送消息。
 这种Source能主动唤醒RunLoop的线程。为什么？
 */
...
/**
 CFRunLoopObserverRef是观察者，也包含了一个回调，当runloop的状态发生改变时，观察者就能通过回调接收到这个变化。可以观察的时间点有CFRunLoopActivity里面的一些枚举。
 */
...
/**
 CFRunLoopTimerRef和NSTimer是toll-free bridged的，可以混用。
 当其加入到runloop中时，runloop会注册对应的时间点，当时间点到时，runloop会被唤醒来执行__CFRunLoopTimer结构体里面的回调。
 */
...
#pragma mark - CFRunLoopGetMain() 和 CFRunLoopGetCurrent()两个函数的大概逻辑
